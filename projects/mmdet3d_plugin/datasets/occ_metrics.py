...
class Metric_mIoU():
    ...
    def count_miou(self):
        ...
        final_mIoU = round(np.nanmean(mIoU[:self.num_classes-1]) * 100, 2)
        final_IoU = round(np.nanmean(IoU[1]) * 100, 2)
        print(f'===> mIoU of {self.cnt} samples: ' + str(final_mIoU))
        print(f'===> IoU of {self.cnt} samples: ' + str(final_IoU))
        # print(f'===> sample-wise averaged mIoU of {cnt} samples: ' + str(round(np.nanmean(mIoU_avg), 2)))

        return self.class_names, mIoU, self.cnt, final_mIoU, final_IoU
