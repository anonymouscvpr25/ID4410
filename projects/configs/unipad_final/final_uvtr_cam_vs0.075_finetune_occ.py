...
frustum_range = [0, 0, 1.0, 1600, 928, 45.0]
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        view_cfg=dict(
            ...
            keep_sweep_dim=True,
            fp16_enabled=False,
            loss_cfg=dict(close_radius=3.0, depth_loss_weights=[0.05]),
        ),
        # transformer_cfg
        in_channels=256,
        loss_occ=dict(
...
dataset_type = "NuScenesSweepDatasetOcc"
...
train_pipeline = [
    ...
    dict(
        type="CollectUnified3D", keys=["points", "img",
                                       "voxel_semantics",
                                       'mask_lidar', 
                                       'mask_camera']),
]
...
data = dict(
    ...
    train=dict(
        ...
        data_root=data_root,
        ann_file=data_root
        + "nuscenes_unified_infos_train_v4_ego.pkl",  # please change to your own info file
    ...
    val=dict(
        ...
        modality=input_modality,
        filter_empty_gt=False,
        ann_file=data_root + "nuscenes_unified_infos_val_v4_ego.pkl",
