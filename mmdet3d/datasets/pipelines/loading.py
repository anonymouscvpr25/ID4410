...
class PointToMultiViewDepth(object):
    ...
    def __call__(self, results):
        ...
        for cid in range(len(results['cams'])):
            lidar2img = torch.from_numpy(results['lidar2img'][cid][0]).float().squeeze(0)  # (4, 4)
...
@PIPELINES.register_module()
class PrepareTemporalData(object):
    ...
    def __call__(self, results):
        ...
        # add some info to future_info
        future_info['pts_filename'] = future_info["lidar_path"]
        future_info['ori_shape'] = results['ori_shape']

        ## Load point clouds and project the points to the image plane to
        ## obtain the depth and semantic maps
        example = self.pipeline(future_info)
        results['render_gt_depth_future'] = example['render_gt_depth']
        results['render_gt_semantic_future'] = example['render_gt_semantic']
        return results